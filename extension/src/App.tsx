    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-white p-6 text-center">
        ...
        <button 
          ...
          className="flex items-center gap-3 bg-gray-900 border border-transparent rounded-full px-6 py-3 text-white font-medium hover:bg-gray-800 transition-all shadow-md hover:shadow-lg active:scale-[0.98] w-full max-w-xs justify-center mb-4"
        >
          <UserCircle className="w-5 h-5 text-gray-300" />
          Continue as Last time
        </button>
            ...
            <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm text-center">
              ...
              <button
                onClick={() => {
                  ...
                  // 【黑客松 Demo 專用】直接塞入 my-custom-job.json 的假資料
                  // 這樣點擊按鈕就會直接把這筆資料送到 Dashboard，不需執行外部 script
                  const mockData = {
                    id: `app-11`,
                    jobTitle: "Engineer I, Field Software",
                    company: "ASM",
                    industry: "Tech",
                    location: "Hillsboro, OR",
                    experienceLevel: "Mid-Level",
                    fitScore: 99,
                    fitBreakdown: { skills: 100, experience: 95, industry: 100, visa: 100, gap: "Perfect Match" },
                    status: "Applied",
                    selectedChannels: ["Company Website"],
                    referral: true,
                    appliedTime: "2026/04/04 10:00", // 確保出現在最上方
                    resumeUrl: "#",
                    coverLetterUrl: "#",
                    notes: [],
                    tags: [],
                    jobDescription: {
                      responsibilities: ["Design and build scalable Machine Learning pipelines.", "Optimize infrastructure for Gemini 3.0 models."],
                      requirements: ["Strong proficiency in Python and C++.", "Experience with large-scale distributed systems."],
                      skills: ["Python", "C++", "PyTorch", "Distributed Systems"],
                      visaInfo: "Sponsorship Available"
                    },
                    interviewTimeline: [],
                    interviewProbability: 0.95,
                    attempts: 1
                  };
                  
                  const token = btoa(encodeURIComponent(JSON.stringify(mockData)));
                  url += `#syncToken=${token}`;
